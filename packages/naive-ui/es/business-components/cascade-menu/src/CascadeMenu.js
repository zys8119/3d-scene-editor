...
import { cascadeMenuLight } from '../styles';
import { useDragDrop } from '../../../_mixins/use-drag-drop';
import { onMounted, watch } from 'vue';
import { useThrottleFn } from '@vueuse/core';
export const cascadeMenuProps = Object.assign(
...
const CascadeMenu = defineComponent({
    name: 'CascadeMenu',
    props: cascadeMenuProps,
    emits: {
    ...
    setup(props, { emit, expose }) {
        ...
        function expandToRoot(node) {
            ...
            set(node, `${parentKey}.${expandedKey}`, true);
            expandToRoot(get(node, parentKey));
        }
                ...
                if (directions.up) {
                    ...
                    if (
                        ...
                        if (targetNode[parentKey]) {
                            set(targetNode, `${parentKey}.${checkedKey}`, true);
            ...
            return {
                ...
                onExpand: (e) =>
                    __awaiter(this, void 0, void 0, function* () {
                        ...
                        const siblings =
                            (_a = get(node, `${parentKey}.${childrenKey}`)) !==
                                null && _a !== void 0
                                ? _a
                                : tree;
            ...
            return Object.assign(
                ...
                useDragDrop({
                    ...
                    onDragover: (e) => {
                        ...
                        const node = get(tree, $el.dataset.path);
                        if (node) {
                    ...
                    onDrop: (e) => {
                        ...
                        const sourceNode = get(tree, sourcePath);
                        const targetNode = get(tree, targetPath);
                        ...
                        emit('node:drag', sourceNode, targetNode, tree, () => {
                            ...
                            const isInSameLevel =
                                get(sourceNode, `${parentKey}.__id__`) ===
                                get(targetNode, `${parentKey}.__id__`);
