...
export const setRoutesName = (routes: RouteRecordRaw[]) => {
    return routes.map((route) => {
        ...
        if (routeMap.component)
            routeMap.component = createAsyncComponent(
                routeMap.name,
                routeMap.component
            );
        ...
        routes.forEach((route) => {
            ...
            if (route.children && !route.meta?.notFlat) {
                flatRoutes(
                    route.children,
                    getPath,
                    newRoute.meta && newRoute.meta.breadcrumbs
                );
            } else {
...
export const setRoutes = async (filter = true) => {
    ...
    const asyncRoutesFilter = filter
        ? await configHooks.router.routesFilter(asyncRoutesWithName)
        : asyncRoutesWithName;
